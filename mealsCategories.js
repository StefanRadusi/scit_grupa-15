...
class PopUp {
  ...
  displayMeals(mealCategory) {
    ...
    for (const meal of mealCategory) {
      ...
      newC.classList.add("pop-up-div")

      const img = document.createElement("img");
      img.src = meal.strMealThumb;
      img.classList.add("pop-up-photos")
      newC.appendChild(img);


      const desc = document.createElement("p");
      desc.innerText = meal.strMeal;
      newC.appendChild(desc);
      desc.classList.add("text-pop-up")


      this.popContent.appendChild(newC);
    }
  }
